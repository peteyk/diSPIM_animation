...
public class Projector {
	...
	private  final String[] methodList = {"Nearest Point", "Brightest Point", "Mean Value"};
	
	private  int axisOfRotationS = yAxis;
	private  int projectionMethodS = brightestPoint;
	private  int initAngleS = 0;
	...
	private boolean showMicroProgress = false;
		...
		for (int c=0; c<channels; c++) {
			for (int f=f1; f<=f2; f++) {
				...
				if (isRGB)
					...
					if (projImpD==null) return projImpD;
	...
	public ImagePlus doProjections(ImagePlus imp) {
		...
		if (imp.getBitDepth()==16 || imp.getBitDepth()==32) {
			...
			for (int i=1; i<=stack.getSize(); i++)
				stack2.addSlice(stack.getProcessor(i).convertToByte(true)); //pete
			stack = stack2;
		}
